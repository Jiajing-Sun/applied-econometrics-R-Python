...
Textbook data: NBS 70-city housing second_hand_yoy indices for 2025.
...
def ridge_fit_predict(
    ...
    yc = y_train - y_mean
    p = Xs_train.shape[1]
    beta = np.linalg.solve(Xs_train.T @ Xs_train + len(y_train) * lam * np.eye(p), Xs_train.T @ yc)
...
def lasso_fit_predict(
    ...
    for _ in range(max_iter):
        ...
        for j in range(p):
            ...
            beta[j] = soft_threshold(rho, n * lam) / col_norm[j] if col_norm[j] > 0 else 0.0
            residual = residual - Xs_train[:, j] * beta[j]
...
def cv_select_lambda(
    ...
    X_train = X[train_idx]
    y_train = y[train_idx]
    months = df["month"].to_numpy()[train_idx]
    rows = []
    for lam in lambdas:
        fold_mse = []
        for validation_month in range(5, 10):
            inner_train = months < validation_month
            inner_test = months == validation_month
            if method == "ridge":
...
df = df.dropna(subset=["second_hand_yoy", "new_house_yoy", "second_hand_mom", "new_house_mom", "first_tier"]).copy()
df = df.sort_values(["city", "month"]).reset_index(drop=True)
df["poly_x"]=(df["new_house_yoy"]-df.loc[df["month"]<=9,"new_house_yoy"].mean())/df.loc[df["month"]<=9,"new_house_yoy"].std(ddof=1)
for i in range(2, 11):
    df[f"new_house_yoy{i}"] = df["poly_x"] ** i

df[
    ["year", "month", "date", "city", "second_hand_yoy", "new_house_yoy", "second_hand_mom", "new_house_mom", "first_tier"]
...
y = df["second_hand_yoy"].to_numpy(float)

random_train_idx = rng.choice(n, size=len(train_idx), replace=False)
random_test_idx = np.setdiff1d(np.arange(n), random_train_idx)
X_split = add_intercept(df[["new_house_yoy", "second_hand_mom", "new_house_mom", "first_tier"]].to_numpy(float))
...
for degree in range(1, 11):
    cols = ["new_house_yoy"] if degree == 1 else ["new_house_yoy"] + [f"new_house_yoy{i}" for i in range(2, degree + 1)]
...
cv_rows = []
X_simple = add_intercept(df[["new_house_yoy"]].to_numpy(float))
for fold, v in enumerate(range(5, 10), start=1):
    leave_out_idx = np.flatnonzero(df["month"].to_numpy() == v)
    leave_in_idx = np.flatnonzero(df["month"].to_numpy() < v)
    beta = fit_ols(y[leave_in_idx], X_simple[leave_in_idx])
    pred = predict_ols(beta, X_simple[leave_out_idx])
    cv_rows.append({"折":fold,"验证月份":v,"MSE":mse(y[leave_out_idx],pred)})
...
X_df = pd.get_dummies(
    df[["new_house_yoy", "second_hand_mom", "new_house_mom", "first_tier", "month_numeric", "city"]],
    columns=["city"],
    drop_first=True,
    dtype=float,
)
X = X_df.to_numpy(float)
lambdas = np.exp(np.linspace(np.log(0.001), np.log(10), 20))
...
tree_features = ["new_house_yoy", "second_hand_mom", "new_house_mom", "first_tier", "month_numeric"]
X_tree = df[tree_features].to_numpy(float)
tree_cv=[]
for depth in (2,3,4,5,6):
    errors=[]
    for v in range(5,10):
        tr=df["month"].to_numpy()<v; va=df["month"].to_numpy()==v
        fit=build_tree(X_tree[tr],y[tr],max_depth=depth,min_leaf=20)
        errors.append(mse(y[va],predict_tree(fit,X_tree[va])))
    tree_cv.append((depth,float(np.mean(errors))))
best_depth=min(tree_cv,key=lambda row: row[1])[0]
pd.DataFrame(tree_cv,columns=["depth","time_cv_mse"]).to_csv(TABLE_DIR/"python_chapter10_tree_cv.csv",index=False)
tree = build_tree(X_tree[train_idx], y[train_idx], max_depth=best_depth, min_leaf=20)
...
for i in range(2, 6):
    df[f"poly{i}"] = df["poly_x"] ** i

X_linear = add_intercept(df[["new_house_yoy", "second_hand_mom", "new_house_mom", "first_tier"]].to_numpy(float))
beta_linear = fit_ols(y[train_idx], X_linear[train_idx])
yhat_ols_linear = predict_ols(beta_linear, X_linear)

X_poly = add_intercept(np.column_stack([X, df[["poly2","poly3","poly4","poly5"]].to_numpy(float)]))
...
display_model = model_compare.iloc[0]  # Prespecified OLS baseline.
...
print(f"Prespecified display model: {display_model['模型']}")
print(f"Prespecified baseline test MSE: {float(display_model['测试集 MSE']):.4f}")
